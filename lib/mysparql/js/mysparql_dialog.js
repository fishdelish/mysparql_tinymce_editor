...
EditorDialog.prototype.submit_callback = function(data) {
  ...
  if (/mysparql/.test(ed.dom.getAttrib(node, 'class'))) {
    ed.dom.setAttrib(node, 'data-mysparql-id', data.mysparql_id);
    ...
    ed.execCommand('mceInsertContent', false, '<div data-formatter="'+formatter+'" data-mysparql-id="'+data.mysparql_id+'" data-xslt-type="'+xslt_type+'" class="mysparql"><a href="'+mysparql_path + 'queries/' + data.mysparql_id+'">MySparql Query</a></div>', {skip_undo : 1});
  }
  tinyMCEPopup.close()
};
