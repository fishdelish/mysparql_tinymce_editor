...
var build_html_link = function(mysparql_id, formatter, xslt_type, parameterised, tutorial, viz) {
...
var update_html_link = function(node, link, mysparql_id, formatter, xslt_type, parameterised, tutorial, viz) {
  var ed = tinyMCEPopup.editor
...
EditorDialog.prototype.submit_callback = function(data) {
  ...
  if (/mysparql/.test(ed.dom.getAttrib(node, 'class'))) {
    update_html_link(node, link, data.mysparql_id, formatter, xslt_type, parameterised, tutorial,viz);
  } else {
    ed.execCommand('mceInsertContent', false, build_html_link(data.mysparql_id, formatter, xslt_type, parameterised, tutorial, viz), {skip_undo:1});
  }
  tinyMCEPopup.close()
};
