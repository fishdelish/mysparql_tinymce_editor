...
var build_html_link = function(mysparql_id, formatter, xslt_type, parameterised, tutorial) {
  var mysparql_string = '<div class="mysparql" data-formatter="' + formatter + '" data-mysparql-id="' + mysparql_id + '" '
  mysparql_string += 'data-xslt-type="' + xslt_type + '" data-parameterised="' + parameterised + '" data-tutorial="' + tutorial + '">'
  mysparql_string += '<a href="' + mysparql_path + 'queries/' + mysparql_id + '">MySparql Query - ' + formatter + '</a></div>'
  alert(mysparql_string)
  return mysparql_string;
};

var update_html_link = function(node, link, mysparql_id, formatter, xslt_type, parameterised, tutorial) {
  var ed = tinyMCEPopup.editor
  ...
  ed.dom.setAttrib(link, 'href', mysparql_path + 'queries/' + data.mysparql_id);
...
EditorDialog.prototype.submit_callback = function(data) {
  ...
  var tutorial = ($('input[name=tutorial]:checked').val() != undefined);
  var parameterised = (/=[0-9a-z_]+=/.test(f['query[source]'].value))
  var link = ed.selection.getNode();
  var node = link.parentNode;
  if (/mysparql/.test(ed.dom.getAttrib(node, 'class'))) {
    update_html_link(node, link, data.mysparql_id, formatter, xslt_type, parameterised, tutorial);
  } else {
    ed.execCommand('mceInsertContent', false, build_html_link(data.mysparql_id, formatter, xslt_type, parameterised, tutorial), {skip_undo:1});
  }
  tinyMCEPopup.close()
};
